...
[![Version](https://img.shields.io/badge/version-1.66.0-blue)](https://github.com/hreskiv/mikr/releases)
...
- **VLAN editing — port PVID (v1.66.0+)** — an Edit mode switch on the Interfaces card, for administrators and operators only and off on every page open, puts a PVID field on each port of a VLAN-filtering bridge. A configuration backup is taken before the change and the change is cancelled if that backup fails; the confirmation states what actually changes, which is not the same sentence for an access port and for a trunk, whose native VLAN moves while its tagged VLANs stay put. The new value is read back from the device afterwards and every change is written to the activity log
