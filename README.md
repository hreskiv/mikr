...
- **VLAN editing (v1.67.0+)** — an Edit mode switch on the Interfaces card, for administrators and operators only and off on every page open, opens a VLAN editor: add the VLANs you need, then give each port a role — **Access** in one VLAN or **Trunk** carrying several, tagged. A switch with no bridge configured yet is a normal starting point: saving creates the bridge, moves the ports into it, writes the VLAN table and enables `vlan-filtering` last, which is the one order that does not interrupt traffic partway through. The interface the panel manages the device through is identified from the device's own IP configuration and kept out of the bridge entirely, so a VLAN change cannot cut the device off. Review changes shows the exact RouterOS commands before anything is written, a configuration backup is taken first and a failed backup cancels the change, and the device is given a rollback that fires in three minutes unless the change is confirmed — closing the browser does not stop it. VLAN names are stored on the device as the VLAN row's comment. A port configured elsewhere with a native VLAN is shown but not editable here, and devices that switch VLANs on the chip rather than on a bridge remain read-only
